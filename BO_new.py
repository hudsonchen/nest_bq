...
def load_ackley(x, dim):
    ...
    y_torch = -ackley(x) # Take the negative of the Ackley function to maximize it
    return y_torch[:, None]
...
def main(args):
    ...
    if args.datasets == 'ackley':
        bounds = torch.tensor([[-32.768], [32.768]]).repeat(1, args.dim)
        train_x = draw_sobol_samples(bounds=bounds, n=1, q=2 * args.dim).squeeze(0)
        train_y = load_ackley(train_x, dim=args.dim)
    ...
    for i in range(args.iterations):
        ...
        new_y = load_ackley(candidate, dim=args.dim)
        ...
        if args.dim == 1:
            ...
            plt.plot(x.numpy(), load_ackley(x, dim=args.dim).numpy(), 'k--', label="Objective")
